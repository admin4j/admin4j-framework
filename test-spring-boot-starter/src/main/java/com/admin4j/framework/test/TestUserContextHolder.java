package com.admin4j.framework.test;
 ...
 */
@Primary
public class TestUserContextHolder implements IUserContextHolder {

    /**
     * 支持父子线程之间的数据传递 THREAD_LOCAL_TENANT
     */
    private static final ThreadLocal<AuthenticationUser> THREAD_LOCAL_USER = new TransmittableThreadLocal<>();

    private static AuthenticationUser authenticationUser;

    static {
        authenticationUser = new AuthenticationUser();
        authenticationUser.setUserId(1L);
        authenticationUser.setTenantId(1L);
        authenticationUser.setUsername("test");
    ...
    public AuthenticationUser getLoginUser() {
        ...
        ResponseEnum.FAIL_AUTH_TOKEN.notNull(authenticationUser);
